...
public extension InteractionEvent {
    func delete(_ completion: ((RequestError?) -> Void)? = nil) {
        self.sword.requestWithBodyAsData(.deleteWebhook(self.sword.user!.id, self.token)) { data, error in
            if let error = error {
              completion?(error)
            } else {
                completion?(nil)
    ...
    mutating func deferReply(_ completion: ((RequestError?) -> Void)? = nil) {
        ...
        let jsonData = try! self.sword.encoder.encode(body)

        self.sword.requestWithBodyAsData(.replyToInteraction(self.interactionId, self.token), body: jsonData) { data, error in
            if let error = error {
              completion?(error)
            } else {
                completion?(nil)
    ...
    func edit(
        ...
        self.sword.requestWithBodyAsData(.editWebhook(self.sword.user!.id, self.token), body: jsonData) { data, error in
            if let error = error {
                  completion?(nil, error)
            } else {
                completion?(Message(self.sword, data as! [String: Any]), error)
            }
        }
    }
    ...
    */
    func reply(
        ...
        if !isDefered {
            ...
            self.sword.requestWithBodyAsData(.replyToDeferedInteraction(self.sword.user!.id, self.token), body: jsonData) { data, error in
                if let error = error {
                  completion?(error)
                } else {
                    completion?(nil)
