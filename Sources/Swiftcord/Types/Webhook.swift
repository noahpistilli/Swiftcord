...
public struct Webhook {
  ...
  */
  ...
  public func delete(then completion: @escaping (RequestError?) -> Void = {_ in}) {
    self.swiftcord?.deleteWebhook(self.id, token: self.token, then: completion)
  }
  ...
  */
  public func executeSlack(
    ...
    self.swiftcord?.executeSlackWebhook(
      self.id,
      token: self.token,
      with: content,
      then: completion
